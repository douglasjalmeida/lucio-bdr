...
3. **Modo mudo durante handoff humano:** tem TRÊS gatilhos, só um depende do envelope da iaSolution. (a) **Closer responde no Chatwoot** → webhook `message_created` outgoing → muta. **VALIDADO em prod 2026-07-17.** (b) **Lúcio qualifica → handoff** → muta. Validado. (c) **Humano responde pelo celular** na coexistência SMB → o eco chega no webhook com `direction !== 'inbound'` → marcar `autor=humano`, NÃO responder, gravar tudo. **Caminho (c) AINDA NÃO validado** (nenhum eco de celular observado; depende de a iaSolution mandar `direction`, campo não documentado — a telemetria de `/health.webhook_iasolution.ultimaMensagem` existe pra flagrar o formato real quando o primeiro chegar).
