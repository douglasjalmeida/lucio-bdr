...
3. **Modo mudo durante handoff humano:** tem TRÊS gatilhos, só um depende do envelope da iaSolution, e **os três estão VALIDADOS em prod (2026-07-17)**. (a) **Closer responde no Chatwoot** → webhook `message_created` outgoing → muta. (b) **Lúcio qualifica → handoff** → muta. (c) **Humano responde pelo celular** na coexistência SMB → o eco chega no webhook com `direction: "outbound"` (+ `user_id`/`to_user_id`, a assinatura do SMB) → `parseMensagemIaSolution` marca `autor=humano`, NÃO responde, grava tudo. Confirmado: msg do celular grava `autor=humano`, o anti-loop NÃO a barra (não tem message_id nosso), e o Lúcio fica calado. O campo `direction` **existe e vem** — a dúvida que motivou a telemetria (`/health.webhook_iasolution.ultimaMensagem`) está resolvida.
4. **Devolução pro bot:** label `devolver-lucio` no Chatwoot → webhook → bridge volta a responder.
5. **Filtro anti-loop:** MEDIDO em 2026-07-17 — **a API oficial NÃO ecoa o texto que sai por ela**, só devolve `statuses` (sent/delivered). O eco que a uazapi dava (e que esta regra afirmava) **não existe pelo caminho da API**. As duas camadas anti-loop — (1) `uazapi_message_id` no Supabase, (2) cache `telefone+conteúdo` 90s — ficam de rede: baratas, e cobrem o eco do CELULAR (coexistência SMB) e uma eventual mudança de comportamento da Meta. Enquanto o único emissor for a API, o risco de "Lúcio se muta sozinho" por eco **não procede**.
